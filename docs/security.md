...
- DeveloperCA審査APIはConsole session、active Account、D1 reviewer allowlist、同一Originをすべて確認します。
- `.mpkg`のmanifest抽出はブラウザー内だけで行い、パッケージ本体をConsole APIへ送信しません。
- manifestのscope・Capabilityは入力補助として扱い、DeveloperCAのD1 Policy外の権限は発行しません。global Capability許可はDeveloper grantと別操作です。
